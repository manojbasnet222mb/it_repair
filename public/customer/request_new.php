<?php
/**
 * Customer Requests — NexusFix (Enterprise Edition)
...
$autosave_key = 'request_draft_' . $user_id;
function save_draft(array $data, string $key) {
    $_SESSION[$key] = $data;
}
function load_draft(string $key): array {
    return $_SESSION[$key] ?? [];
}
function clear_draft(string $key) {
    unset($_SESSION[$key]);
}

// Load draft if exists
if ($_SERVER['REQUEST_METHOD'] === 'GET') {
    $draft = load_draft($autosave_key);
    if (!empty($draft)) {
        $old = array_merge($old, $draft);
    }
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  if ($old['serial_no'] === '') $errors['serial_no'] = 'Serial / IMEI is required.';
  if (!valid_in(['dropoff','pickup','onsite'], $old['service_type'])) $errors['service_type'] = 'Invalid service option.';
  if (!valid_in(['phone','email'], $old['preferred_contact'])) $errors['preferred_contact'] = 'Choose phone or email.';
  ...
  if (!$errors && isset($_POST['confirm']) && $_POST['confirm'] === 'yes') {
    try {
      ...
      $hist = $pdo->prepare("INSERT INTO request_status_history (request_id, status, note, changed_by) VALUES (?,?,?,NULL)");
      $hist->execute([$req_id, 'Received', 'Request submitted by customer']);
      ...
      if (!empty($confirmation_data['service_date']) && !empty($confirmation_data['service_time_period'])) {
          ...
          $insB->execute([$bookingIdentifier, $req_id, $_SESSION['user']['id'], 'booked', 'Customer booked date/time period during request creation']);
      }
      ...
      $n->execute([$_SESSION['user']['id'], 'Request Received', "Ticket $ticket has been created."]);
      
      // Clear draft after successful submission
      clear_draft($autosave_key);
      
      $pdo->commit();
      redirect(base_url('customer/requests.php?new='.$ticket));
    } catch (Throwable $e) {
...
<head>
  ...
  <meta name="description" content="Start a new repair request with NexusFix. Upload photos, describe issues, and book service slots.">
  <title>New Repair Request — NexusFix</title>
...
<body>
  ...
  <div>
  <h2 id="page-title">New Repair Request</h2>
  <p class="subtitle">Describe your device issue, attach photos, and choose your preferred service option.</p>
</div>
  <?php if ($errors): ?>
    ...
    <?php if ($confirming): ?>
        ...
        <form method="post" enctype="multipart/form-data" novalidate>
            ...
            <?php foreach ($confirmation_data as $k => $v): ?>
                <?php if ($k !== 'slot_details' && $k !== 'temp_files'): ?>
            ...
            <div class="actions">
                ...
                <a href="<?= e(base_url('customer/dashboard.php')) ?>" class="btn subtle">Cancel</a>
            </div>
        </form>
    <?php else: ?>
        <!-- Main Form -->
        <form method="post" enctype="multipart/form-data" novalidate id="requestForm">
          ...
          <section class="card">
            ...
            <div class="grid grid-4">
              ...
              <label for="serial_no">Serial / IMEI *
                <input type="text" id="serial_no" name="serial_no" value="<?= e($old['serial_no']) ?>" placeholder="Required" maxlength="120" required>
                <?php if (!empty($errors['serial_no'])): ?>
                    <div style="color:var(--danger); font-size:0.85rem; margin-top:4px;"><?= e($errors['serial_no']) ?></div>
                <?php endif; ?>
              </label>
            </div>
          </section>
          ...
          <section class="card">
            ...
            <div class="autosave-indicator" id="autosaveIndicator">Draft saved</div>
          </section>
          <!-- Attachments -->
          <section class="card">
    ...
    <div style="grid-column: span 2;">
      ...
      <?php endif; ?>
          ...
          <div class="actions">
            ...
            <button type="button" class="btn subtle" id="saveDraftBtn">Save Draft</button>
            <a href="<?= e(base_url('customer/dashboard.php')) ?>" class="btn subtle">Cancel</a>
          </div>
        </form>
    <?php endif; ?>
  ...
  <script>
    // --- Issue Chips ---
    const issueOptions = {
      "Phone": ["Cracked screen","Battery drains fast","Charging port issue","Liquid damage","Camera not working","Wi-Fi/Bluetooth issue"],
      "Laptop": ["No power","Blue screen","Overheating","Slow performance","Keyboard issue","No display","Virus/malware"],
      "Tablet": ["Cracked screen","Battery issue","Touch unresponsive","Wi-Fi not working"],
      "Desktop": ["No power","No display","Slow performance","Disk failure"],
      "Printer": ["Paper jam","Not printing","Ink issue","Connectivity problem"],
      "Peripheral": ["Not detected","Connection issues","Button failure"],
      "Other": ["Unidentified issue","Diagnostics needed"]
    };
    ...
    document.querySelectorAll('#requestForm input, #requestForm select, #requestForm textarea').forEach(element => {
